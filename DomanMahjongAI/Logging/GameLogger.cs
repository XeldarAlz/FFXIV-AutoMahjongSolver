...
namespace DomanMahjongAI.Logging;
...
///
/// Output: <c>%AppData%\XIVLauncher\pluginConfigs\DomanMahjongAI\games\game-YYYYMMDD-HHMMSS-handNN.ndjson</c>.
///
...
public sealed class GameLogger : IDisposable
{
    ...
    public GameLogger(StateAggregator aggregator, Configuration config)
    {
        this.aggregator = aggregator;
        this.config = config;
        var root = Plugin.PluginInterface.GetPluginConfigDirectory();
        gamesDir = Path.Combine(root, "games");
    ...
    private void OnStateChanged(StateSnapshot snap)
    {
        ...
        {
            Plugin.Log.Error($"GameLogger state-write error: {ex.Message}");
        }
    }
    ...
    {
        if (!config.EnableGameLogging || disposed) return;
        try
        {
        ...
        {
            Plugin.Log.Error($"GameLogger action-write error: {ex.Message}");
        }
    }
    ...
    {
        lock (writerLock) CloseWriterLocked();
    }
    ...
    {
        lock (writerLock) { writer?.WriteLine(line); }
    }
